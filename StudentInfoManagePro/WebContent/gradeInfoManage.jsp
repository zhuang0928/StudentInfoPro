...
<script type="text/javascript">
	...
	function openGradeModifyDialog() {
		...
		var selectedRow = $("#dg").datagrid('getSelections');//返回多条记录
		if (selectedRow.length != 1) {
			$.messager.alert("系统提示", "请选择一条要编辑的数据！");
			return;
		}
		var row = selectedRow[0];
		$('#dlg').dialog('open').dialog('center').dialog('setTitle', '编辑班级信息');
		$('#fm').form('load', row);
...
<body style="margin: 5px">
	...
	<div id="tb">
		...
		<div>
			班级名称：<input type="text" name="s_gradeName" id="s_gradeName" /> <a
	...
	<div id="dlg-buttons">
		<a href="javascript:saveGrade()" class="easyui-linkbutton"
			...
			class="easyui-linkbutton" iconCls="icon-cancel" s>关闭</a>
	</div>
